...
export const searchParamsSchema = z.object({
  ...
  ownerNameMatchType: matchTypeSchema.default('exact').optional(),
  ownerAddress: z.string().optional(),
  ownerAddressMatchType: matchTypeSchema.default('partial').optional(),
  searchSites: z.array(z.string()).optional(),
  siteSearchMode: z.enum(["any", "specific", "exclude"]).default("any").optional(),
  isAdvancedSearchEnabled: z.string().transform(val => val === 'true').optional(),
  count: z.coerce.number().min(1).max(20).default(20).optional(),
  offset: z.coerce.number().min(0).max(9).default(0).optional(),
  page: z.coerce.number().min(1).default(1).optional(),
  safeSearch: z.enum(['off', 'moderate', 'strict']).default('moderate').optional(),
  searchType: z.enum(['web', 'news', 'images']).default('web').optional(),
  domain: z.string().optional(),
  period: z.enum(['all', 'last_6_months', 'last_year', 'last_3_years', 'last_5_years', 'last_10_years']).default('all').optional(),
});
