...
import { getProjectAction, getProjectPropertiesAction, getProjectOwnersAction, getProjectStatsAction } from "./action";
      ...
      <div className="py-8">
        ...
        {stats && (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-muted/50 rounded-lg p-4">
              <div className="text-sm text-muted-foreground mb-2">所有者調査進捗</div>
      ...
      <Tabs defaultValue={view} className="pb-10">
        <div className="flex items-center justify-between mb-6">
          <TabsList className="grid w-[400px] grid-cols-2">
            <TabsTrigger value="properties" asChild>
              <Link href={`/projects/${projectId}?view=properties`} className="flex items-center gap-2">
                <LayoutGrid className="size-4" />
                物件ベース表示
              </Link>
            </TabsTrigger>
            <TabsTrigger value="owners" asChild>
              <Link href={`/projects/${projectId}?view=owners`} className="flex items-center gap-2">
                <Users className="size-4" />
                所有者ベース表示
              </Link>
            </TabsTrigger>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            {!isPropertyView && owners ? `全${new Set(owners.map(o => o.owner.id)).size}名の所有者` : ''}
        ...
        <TabsContent value="owners" className="mt-0">
          {ownersError ? (
            ...
            <div className="flex items-center justify-center h-32">
              <p className="text-muted-foreground">所有者データを読み込み中...</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
