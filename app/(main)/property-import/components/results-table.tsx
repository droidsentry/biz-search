...
import { Check, Eye, Loader2, MapPin, MapPinned, MapPlus, MoreHorizontal, MoreVertical, RefreshCw, Trash2, X, ExternalLink } from 'lucide-react'
...
export function ResultsTable({ results, onReset, onDelete }: ResultsTableProps) {
  const successCount = results.filter(r => r.status === 'success').length
  const totalProperties = results.reduce((sum, r) => 
    sum + (r.propertyData?.length || 0), 0
  )
  
  // ジオコーディング結果の状態管理
  const [geocodingResults, setGeocodingResults] = useState<Map<string, GeocodingResult>>(new Map())
  ...
  const tableRows: TableRow[] = results.flatMap((result, index): TableRow[] => {
    if (result.propertyData && result.propertyData.length > 0) {
      return result.propertyData.map((property, propIndex): TableRow => ({
                ...
                <div className="mt-2 pt-2 border-t border-muted-foreground/20">
                  ...
                  {geocoding.lat && geocoding.lng ? (
                    <>
                      ...
                      <p>ストリートビュー: {geocoding.streetViewAvailable ? '利用可能' : '利用不可'}</p>
                    </>
                  ) : geocoding.error ? (
  ...
  const handleBatchGeocode = async () => {
    // 有効な物件データを持つ行のみを対象
    const validRows = tableRows.filter(row => row.property && row.property.ownerAddress)
    ...
    try {
      ...
      toast.success(`${batchResults.filter(r => r.success).length}件の位置情報を取得しました`)
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-400">
            {successCount}/{results.length}件のPDFを解析 • {totalProperties}件の物件情報を抽出
          </p>
        </div>
          ...
          >
            {isGeocoding ? (
              <>
                ...
                {geocodingProgress ? `${geocodingProgress.current}/${geocodingProgress.total}件処理中` : '処理中...'}
              </>
            ) : (
              <>
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-zinc-300">{formatFileSize(row.fileSize)}</span>
              </div>
              ...
              {row.property && (
                <>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="-mx-4">
                      <CopyCell value={row.property.propertyAddress} className="text-white font-medium" truncate={false} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="-mx-4">
                      <CopyCell value={row.property.ownerName} className="text-zinc-300" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="-mx-4">
                      <CopyCell value={row.property.ownerAddress} className="text-zinc-300" truncate={false} />
                    </div>
                  </div>
                  ...
                  {geocoding && (
                    <div className="flex items-center gap-4 pt-2">
                      <div className="flex items-center gap-2">
                        ...
                        {geocoding.loading ? (
                          ...
                          <Button
                            ...
                            size="sm"
                            className="h-7 px-2"
                            onClick={() => window.open(generateMapsUrl(geocoding.lat!, geocoding.lng!), '_blank')}
                          >
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-zinc-500">ストリートビュー:</span>
                        {geocoding.streetViewAvailable === true && geocoding.lat && geocoding.lng ? (
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 px-2"
                            onClick={() => {
                              window.open(generateStreetViewUrl(geocoding.lat!, geocoding.lng!), '_blank')
                            }}
                          >
                            <ExternalLink className="size-3 mr-1" />
                            開く
        ...
        <Table className="table-fixed">
          <TableHeader>
            <TableRow className="border-zinc-800 hover:bg-transparent">
              ...
              <TableHead className="text-foreground/80 font-semibold w-60">物件住所</TableHead>
              <TableHead className="text-foreground/80 font-semibold w-25">所有者名</TableHead>
              <TableHead className="text-foreground/80 font-semibold w-60">所有者住所</TableHead>
              ...
              >
                ...
                <TableCell className="text-foreground/80 p-0">
                  {row.property ? (
                    ...
                    <span className="block truncate px-2 py-1" title={row.error || 'エラー'}>
                      {row.error || 'エラー'}
                    </span>
                  )}
                </TableCell>
                  ...
                  {row.property && (
                    geocoding?.loading ? (
                      ...
                      <Button
                        ...
                        asChild
                      >
                        <Link href={generateMapsUrl(geocoding.lat!, geocoding.lng!)} target="_blank">
                          <ExternalLink/>
                        </Link>
                      </Button>
                    ) : geocoding?.error ? (
                  ...
                  {row.property && geocoding && (
                    geocoding.streetViewAvailable === true && geocoding.lat && geocoding.lng ? (
                      <Button
                        ...
                        asChild
                      >
                        <Link href={generateStreetViewUrl(geocoding.lat!, geocoding.lng!)} target="_blank">
                          <ExternalLink/>
                        </Link>
                      </Button>
                    ) : geocoding.streetViewAvailable === false ? (
