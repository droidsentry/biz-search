...
import type { ApiStats } from "../actions/api-usage";
import { getAllApiUsageStats } from "../actions/api-usage";
        ...
        <CardContent className="pt-2 pb-4">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {(() => {
                const selectedStats = apiStats.find(
                  (s) => s.apiName === selectedApi
                );
                      ...
                      <div className="flex items-center justify-between">
                            ...
                            {(() => {
                              const currentMonth =
                                selectedStats.monthly[
                                  selectedStats.monthly.length - 1
                                ];
                              const previousMonth =
                                selectedStats.monthly[
                                  selectedStats.monthly.length - 2
                                ];
                              if (
                                currentMonth &&
                                previousMonth &&
                                previousMonth.requests > 0
                              ) {
                                const change =
                                  ((currentMonth.requests -
                                    previousMonth.requests) /
                                    previousMonth.requests) *
                                  100;
                                return (
                                  <>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-sm font-semibold text-gray-900 dark:text-gray-100">
                            {(() => {
                              const limit =
                                apiStats.find((s) => s.apiName === selectedApi)
                                  ?.daily.limit || 0;
