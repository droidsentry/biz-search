...
interface PdfUsageChartProps {
  used: number
  limit: number
  status: 'active' | 'warning' | 'danger' | 'inactive'
}

export function PdfUsageChart({ used, limit, status }: PdfUsageChartProps) {
  ...
  const getStatusBadge = (status: PdfUsageChartProps['status']) => {
    switch (status) {
      case 'active':
        return (
      ...
      <div className="mt-4 space-y-3 w-full">
        ...
        <div className="flex items-center justify-between text-sm px-2">
          <div className="flex items-center gap-2">
            <span className="text-muted-foreground">残り処理可能:</span>
            <span className="font-medium">{(limit - used).toLocaleString()}件</span>
          </div>
        ...
        {percentage > 90 && (
          <div className="mt-2 p-2 bg-red-50 dark:bg-red-900/10 rounded-lg">
            <div className="flex items-center gap-2 text-red-700 dark:text-red-400 text-xs">
              ...
              <span className="font-medium">処理制限に近づいています</span>
            </div>
          </div>
        )}
      </div>
