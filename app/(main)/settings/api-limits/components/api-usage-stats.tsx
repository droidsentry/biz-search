...
export function ApiUsageStats({ usage, limits }: ApiUsageStatsProps) {
      ...
      .on(
        ...
        (payload) => {
          if (payload.new && typeof payload.new === "object") {
            setStats(payload.new as Tables<"api_global_usage">);
          }
        }
      )
      .subscribe();
  ...
  const handleReset = async (type: "daily" | "monthly" | "unblock") => {
    ...
    try {
      const supabase = createClient();
      type UpdateData = Partial<Tables<"api_global_usage">>;
  ...
  const dailyPercentage = limits
    ? ((stats?.daily_count || 0) / limits.daily_limit) * 100
    : 0;
  const monthlyPercentage = limits
    ? ((stats?.monthly_count || 0) / limits.monthly_limit) * 100
    : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>現在のAPI使用状況</CardDescription>
      </CardHeader>
          ...
          <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-4">
            ...
            {stats.blocked_until && (
              <p className="text-sm text-muted-foreground mt-1">
                解除予定:{" "}
                {new Date(stats.blocked_until).toLocaleString("ja-JP")}
              </p>
            )}
          </div>
        )}
