-- get_project_properties_view関数を修正してprofilesテーブルからusernameを取得
...
BEGIN
  ...
  WITH property_owners AS (
    ...
    SELECT 
      ...
      o.updated_by AS owner_updated_by,
  ...
  primary_owners_with_company AS (
    ...
    GROUP BY po.pp_id, po.prop_id, po.prop_address, po.added_date, 
             ...
             po.owner_updated_at, po.owner_updated_by,
             po.owner_rank, oc.id, oc.company_name, oc.updated_at, oc.researched_by
  )
  -- 最終結果を構築
  SELECT 
    ...
    p_owner.username AS primary_owner_updated_by_username,  -- profilesから取得
    ...
    p_company.username AS primary_company_researched_by_username,  -- profilesから取得
  ...
  LEFT JOIN profiles p_owner ON pow.owner_updated_by = p_owner.id
  LEFT JOIN profiles p_company ON pow.company_researched_by = p_company.id
