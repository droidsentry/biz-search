-- get_project_properties_view関数にupdated_atフィールドを追加
...
RETURNS TABLE(
  ...
  primary_owner_updated_at timestamp with time zone,  -- 追加
  primary_company_id uuid, 
  primary_company_name text, 
  primary_company_updated_at timestamp with time zone,  -- 追加
...
BEGIN
  ...
  WITH property_owners AS (
    ...
    SELECT 
      ...
      o.updated_at AS owner_updated_at,  -- 追加
  ...
  primary_owners_with_company AS (
    ...
    SELECT 
      ...
      oc.id AS company_id,
      oc.company_name,
      oc.updated_at AS company_updated_at,  -- 追加
    ...
    GROUP BY po.pp_id, po.prop_id, po.prop_address, po.added_date, 
             ...
             po.owner_updated_at,  -- 追加
             po.owner_rank, oc.id, oc.company_name, oc.updated_at  -- 追加
  )
  -- 最終結果を構築
  SELECT 
    ...
    pow.owner_updated_at AS primary_owner_updated_at,  -- 追加
    pow.company_id AS primary_company_id,
    pow.company_name AS primary_company_name,
    pow.company_updated_at AS primary_company_updated_at,  -- 追加
