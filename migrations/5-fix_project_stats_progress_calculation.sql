-- Fix progress calculation to include 'unknown' status as completed
-- Description: Modify owner_progress calculation to include both 'completed' and 'unknown' statuses
-- Date: 2025-08-02

-- Drop existing function
DROP FUNCTION IF EXISTS public.get_project_stats(uuid);

-- Create updated function with fixed progress calculation
...
BEGIN
  ...
  SELECT
    ...
    CASE
      ...
      -- completedとunknownの両方を調査済みとしてカウント
...
-- Add comment to document the updated calculation logic
COMMENT ON FUNCTION public.get_project_stats(uuid) IS 'プロジェクトの統計情報を取得。進捗率はcompletedとunknownの両方を調査済みとして計算';