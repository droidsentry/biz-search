-- get_projects_with_full_stats関数のupdated_atカラムの曖昧さを修正

-- 既存の関数を削除
DROP FUNCTION IF EXISTS public.get_projects_with_full_stats();

-- 新しい関数を作成（updated_atカラムを明確に指定）
...
RETURNS TABLE(
  ...
  total_owners bigint,
  pending_owners bigint,
  completed_owners bigint,
  unknown_owners bigint,
...
BEGIN
  ...
  project_owners AS (
    ...
    SELECT DISTINCT
      ...
      o.updated_at AS owner_updated_at,  -- 明確に指定
      o.updated_by AS owner_updated_by   -- 明確に指定
    FROM projects p
  ...
  project_owner_stats AS (
    ...
    SELECT
      ...
      MAX(owner_updated_at) AS last_owner_updated_at,
      -- 最終更新日時に対応する更新者を取得
      (SELECT owner_updated_by FROM project_owners po2 
       WHERE po2.project_id = project_owners.project_id 
       AND po2.owner_updated_at = MAX(project_owners.owner_updated_at)
  ...
  project_company_updates AS (
    SELECT DISTINCT
      p.id AS project_id,
      oc.updated_at AS company_updated_at,  -- 明確に指定
      oc.researched_by AS company_researched_by
    FROM projects p
    INNER JOIN project_properties pp ON p.id = pp.project_id
    INNER JOIN property_ownerships po ON pp.property_id = po.property_id
  ...
  project_company_stats AS (
    SELECT
      project_id,
      MAX(company_updated_at) AS last_company_updated_at,
      (SELECT company_researched_by FROM project_company_updates pcu2
       WHERE pcu2.project_id = project_company_updates.project_id
       AND pcu2.company_updated_at = MAX(project_company_updates.company_updated_at)
